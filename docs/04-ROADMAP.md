...
| 8 | Admin Panel (includes matchmaking-staff console — replaces former "Agent Portal" phase) | ✅ Backend done — `backend/src/admin`: dedicated `POST /admin/login` (rejects regular-user credentials; `/auth/login` conversely rejects staff), `super_admin`-only staff provisioning (`POST /admin/staff`, restricted to support_staff/matchmaking_staff/admin — **not** super_admin, which requires the `npm run seed:super-admin` script), user list + suspend/reactivate, profile verification queue + approve/reject, staff-assignment endpoint, and an append-only audit log wired to every privileged action above. First real use of `RolesGuard`/`@Roles()`. Verified end-to-end: login separation both directions, role-gated 403s, verification workflow, audit trail contents. UI still pending (frontend has no admin views yet — see phase 5/6) |
...
12. ~~Admin Panel core~~ ✅ Done: dedicated `/admin/login`, staff provisioning, user suspend/reactivate, profile verification queue, staff assignment, audit log (see phase 8 above). Still missing: CMS (blogs/testimonials/FAQ/banners) and the analytics dashboards from the SRS (§4.7) — those weren't part of this pass.
...
17. Frontend for search/matches/chat — the dashboard only covers auth + own-profile CRUD right now. There's no UI yet for browsing search results, sending/responding to interests, or the chat screen, even though all three backends are fully built and tested.
18. Angular environment files (`fileReplacements` in `angular.json`) to replace the hardcoded `API_BASE_URL` constant, and Angular Material (still not installed — Tailwind only so far).
